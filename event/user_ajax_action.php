...
if(isset($_POST['page']))
{
	...
	if($_POST['page'] == 'index')
	{
		...
		{
		    //$email = Get_user($_SESSION['user_id'],'email');
		    
		    //$res = mysqli_query($con,"SELECT * FROM algo_round_2 WHERE email = '$email'");
		    ...
		    else
		    {
			    ...
			    <div class="card">
				    <div class="card-header">Exam Details</div>
				        <div class="card-body">
				            <h1> Only Qualified For Algoholics. </h1>
				        </div>
				    </div>
				</div>    
			';
		...
		if($_POST['action'] == 'enroll_exam')
		{
			...
			$result = mysqli_query($con,"SELECT question_id FROM question_table WHERE online_exam_id = '".$_POST['exam_id']."'");
			while($row = mysqli_fetch_assoc($result))
			{
				...
				mysqli_query($con,"INSERT INTO user_exam_question_answer (`user_id`,`exam_id`,`question_id`, `user_answer_option`, `marks`) 
	...
	if($_POST["page"] == 'enroll_exam')
	{
		if($_POST['action'] == 'fetch')
		{
			...
			foreach($result as $row)
			{
				...
				if($row["online_exam_status"] == 'Started')
				{
					$view_exam = '<a href="get_exam.php?code='.$row["online_exam_code"].'" class="btn btn-info btn-sm">View Exam</a>';
				}
				if($row["online_exam_status"] == 'Completed')
				{
					$view_exam = '<a href="success.php?code='.$row["online_exam_code"].'" class="btn btn-info btn-sm">View Exam</a>';
				}
	...
	if($_POST['page'] == 'view_exam')
	{
		if($_POST['action'] == 'load_question')
		{
			...
			foreach($result as $row)
			{
				...
				{
					$output .= '
					<img src="'.$row["question_title"].'" style="font-size: 2rem;" />
					<hr />
					<br />
					<div class="row">
					';
				...
				{
					$output .= '
					<h1 style="font-size: 2rem;">'.$row["question_title"].'</h1>
					<hr />
					<br />
					<div class="row">
					';
				...
				$count = 1;
				$option = $exam->Get_user_exam_answer($row["online_exam_id"],$_SESSION['user_id'],$row["question_id"]);
				$option_checked = '';
				foreach($sub_result as $sub_row)
				{
					...
					<div class="col-md-6" style="margin-bottom:32px;">
						<div class="radio">
							<label><h4 style="font-size:1.6rem;"><input type="radio" name="option_1" class="answer_option" '.$option_checked.' data-question_id="'.$row["question_id"].'" data-id="'.$count.'"/>&nbsp;'.$sub_row["option_title"].'</h4></label>
						</div>
					</div>
					';
				...
				$if_previous_disable_css = '';
				$if_next_disable_css = '';

				if($previous_id == "")
				{
					$if_previous_disable = 'disabled';
					$if_previous_disable_css = "opacity-50 cursor-not-allowed";
				}
				
				if($next_id == "")
				{
					$if_next_disable = 'disabled';
					$if_next_disable_css = "opacity-50 cursor-not-allowed";
				}

				$submit = '';
				if($if_next_disable == 'disabled')
				{
					$submit = '<button type="button" name="submit" class="bg-green-500 hover:bg-green-700 text-white font-bold py-2 px-4 rounded submit">Finish</button> ';
				}

				$output .= '
					<br /><br />
				  	<div align="center">
				   		<button type="button" name="previous" class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded '.$if_previous_disable_css.' previous" id="'.$previous_id.'" '.$if_previous_disable.'>Previous</button>
				   		<button type="button" name="next" class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded '.$if_next_disable_css.' next" id="'.$next_id.'" '.$if_next_disable.'>Next</button>
						'.$submit.'
				  	</div>
				  	<br /><br />';
			}
		...
		if($_POST['action'] == 'question_navigation')
		{
			...
			foreach($result as $row)	
			{
				...
				<div class="col-md-2" style="margin-bottom:24px;">
					<button type="button" style="padding-left: 1rem !important;padding-right: 1rem !important;" class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 rounded question_navigation" data-question_id="'.$row["question_id"].'">'.$count.'</button>
				</div>
				';
				$count++;
			}
		...
		if($_POST['action'] === "check_limit")
		{
			$exam->query = "
			SELECT * FROM user_exam_enroll_table 
			WHERE user_id = '".$_SESSION["user_id"]."' AND exam_id = '".$_POST['exam_id']."'
			";

			$result = $exam->query_result();

			echo $result[0]['error_limit'];
		}

		if($_POST['action'] === "update_limit")
		{
			$exam->query = "
			UPDATE user_exam_enroll_table SET error_limit = error_limit - '1'  
			WHERE user_id = '".$_SESSION["user_id"]."' AND exam_id = '".$_POST['exam_id']."'
			";
